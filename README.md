...
4. 봇 토큰은 **환경변수로만** 저장(코드·config에 넣지 않음):
   ```powershell
   setx TELEGRAM_BOT_TOKEN "123456:ABC-..."   # 새 콘솔부터 적용
   ```
...
- 토큰(env)이나 chat_id가 없으면 조용히 skip(노트는 정상 생성).
- 여러 프로필이 서로 다른 사람에게 가야 하면 config마다 `chat_id`를 다르게 준다(봇 토큰은 공용).
- 작업 스케줄러로 돌릴 땐 같은 사용자 계정이라 `setx`로 넣은 환경변수를 그대로 읽는다.
